...
# === Setup ===
install:
	uv sync --python 3.12 --directory backend --dev
...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend red_door db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend red_door db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend red_door db upgrade

db-downgrade:
	uv run --directory backend red_door db downgrade

db-current:
	uv run --directory backend red_door db current

db-history:
	uv run --directory backend red_door db history

# === Server ===
run:
	uv run --directory backend red_door server run --reload --port 8008

run-prod:
	uv run --directory backend red_door server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend red_door server routes
...
worker-prod:
	uv run --directory backend arq app.worker.arq_app.WorkerSettings --watch app/worker

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend red_door user create-admin

user-create:
	uv run --directory backend red_door user create

user-list:
	uv run --directory backend red_door user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/red_door"
...
help:
	@echo ""
	@echo "red_door - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
