...
Each generated project includes a CLI tool named `red_door`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run red_door server run --reload     # Start dev server
uv run red_door server routes           # Show all routes
# Database commands
uv run red_door db migrate -m "message" # Create migration
uv run red_door db upgrade              # Apply migrations
uv run red_door db downgrade            # Rollback migration
# User commands
uv run red_door user create-admin       # Create admin user
uv run red_door user create             # Create regular user
uv run red_door user list               # List all users
```
...
# Server
uv run red_door server run --reload
uv run red_door server routes

# Database (Alembic wrapper)
uv run red_door db init
uv run red_door db migrate -m "Add users"
uv run red_door db upgrade

# Users
uv run red_door user create-admin       # Create admin (interactive)
uv run red_door user create             # Create user (interactive)
uv run red_door user list               # List all users
```
...
```bash
uv run red_door cmd seed --count 100
uv run red_door cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend red_door server run --reload` |
| Start prod server | `uv run --directory backend red_door server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend red_door server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend red_door db migrate -m "message"` |
| Apply migrations | `uv run --directory backend red_door db upgrade` |
| Rollback migration | `uv run --directory backend red_door db downgrade` |
| Show current | `uv run --directory backend red_door db current` |
| Show history | `uv run --directory backend red_door db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend red_door user create-admin` |
| Create user | `uv run --directory backend red_door user create` |
| List users | `uv run --directory backend red_door user list` |
