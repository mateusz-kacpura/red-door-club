...
class TestSettings:
    ...
    def test_project_name_is_set(self):
        ...
        assert settings.PROJECT_NAME == "red_door"
...
class TestCacheSetup:
    """Tests for Redis client setup."""

    def test_redis_client_class_exists(self):
        """Test RedisClient class is importable."""
        from app.clients.redis import RedisClient

        assert RedisClient is not None
        assert callable(RedisClient)

    def test_redis_client_has_required_methods(self):
        """Test RedisClient exposes the expected async interface."""
        from app.clients.redis import RedisClient

        for method in ("connect", "close", "get", "set", "delete", "exists", "ping", "publish"):
            assert callable(getattr(RedisClient, method, None)), f"Missing method: {method}"
