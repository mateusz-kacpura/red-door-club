...
async def verify_reader_api_key(
    x_api_key: Annotated[str | None, Header(alias="X-API-Key")] = None,
) -> None:
    """Verify NFC reader API key from X-API-Key header.

    If NFC_READER_API_KEY is not configured in settings, all requests pass through
    (public/unauthenticated access). When configured, the header must match exactly.
    """
    configured_key = settings.NFC_READER_API_KEY
    if configured_key and x_api_key != configured_key:
        raise AuthorizationError(message="Invalid or missing X-API-Key")


ReaderApiKey = Annotated[None, Depends(verify_reader_api_key)]
