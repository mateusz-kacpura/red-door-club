...
from app.clients.arq import get_arq_pool
...
class UserService:
    ...
    async def register(self, user_in: UserCreate) -> User:
        ...
        try:
            ...
            await pool.enqueue_job("process_user_matching", str(user.id))
        except Exception as e:
            logger.warning(f"Failed to enqueue matching task: {e}")
    ...
    async def update(self, user_id: UUID, user_in: UserUpdate) -> User:
        ...
        segment_fields = {"interests", "industry", "revenue_range", "user_type"}
        if segment_fields & set(update_data.keys()):
            try:
                pool = await get_arq_pool()
                await pool.enqueue_job("process_user_matching", str(updated_user.id))
            except Exception as e:
                logger.warning(f"Failed to enqueue matching task: {e}")
